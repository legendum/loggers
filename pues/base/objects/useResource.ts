...
export function useResource<TExtra = Record<string, unknown>>(
  ...
  const buildUrl = useCallback(
    (extras?: Record<string, string | number>) => {
      ...
      // Re-parse from filtersKey so this callback's deps stay stable even
      // when the caller passes a fresh `filters` object every render (the
      // common case: `filters: { status, q }` constructed inline). The
      // serialized form is the only safe identity for the dep array.
      if (filtersKey) {
        const parsed = JSON.parse(filtersKey) as Record<
          string,
          string | number
        >;
    ...
    [basePath, name, paginated, pageSize, filtersKey],
  );
  ...
  const handlers = useMemo(() => {
    ...
    return {
      ...
      // SPEC §7 — `resync` is a per-stream event from `sseRoute` when the
      // client's Last-Event-ID is outside the server's ring buffer (stale
      // or pre-restart). Drop the cache and refetch from REST so it lines
      // up with current truth. No event-name prefix: `resync` is a stream
      // primitive, not a resource event.
      resync: () => {
        setReloadTick((n) => n + 1);
      },
    };
